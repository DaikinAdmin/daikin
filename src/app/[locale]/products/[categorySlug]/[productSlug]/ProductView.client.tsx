...
import { Product, ProductImage, Feature, Spec, ProductItem } from "@/types/product";
...
export default function ProductView({ product, locale }: Props) {
  ...
  // normalize arrays
  const images: ProductImage[] = (product.images ?? product.img ?? []) as ProductImage[];
  const features: Feature[] = product.features ?? [];
  const specsAll = product.specs ?? [];
  const items: ProductItem[] = product.items ?? [];
  ...
  return (
    <main className="max-w-7xl mx-auto px-2 md:px-4 lg:px-8 py-8 flex flex-col gap-4 md:gap-8">
              ...
              <div className="flex w-full items-center gap-2 mt-2 md:w-auto md:justify-end">
                <div className="flex flex-row-reverse md:flex-row items-center gap-2">
                  ...
                  <div className="flex gap-2">
                    {images.map((c: ProductImage, idx: number) => (
                      <button
                        key={c.id ?? idx}
                        className={`relative w-12 h-12 md:w-14 md:h-14 rounded-full p-[3px] border-2 border-primary ${
                          selectedColorIndex === idx ? "border-opacity-100" : "border-opacity-0"
                        }`}
      ...
      {/* Includes / Items */}
      <div className="mt-16">
        <h2 className="text-h1-mobile md:text-h1 mb-6">{t("itemsTitle") ?? "Included"}</h2>
        <EmblaCardCarousel
          items={
            items.map((it: ProductItem) => ({
              img: it.img ?? "",
              title: it.productItemDetails?.find((d) => d.locale === locale)?.title ?? it.title ?? "",
              subtitle: it.productItemDetails?.find((d) => d.locale === locale)?.subtitle ?? "",
            })) || []
          }
        />
      </div>
